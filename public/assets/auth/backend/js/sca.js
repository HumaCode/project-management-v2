...
(function (global) {
  ...
  ───────────────────────────────────────────────────── */
    function buildDialog(opts) {
        ...
        var showCancel =
            opts.showCancel !== false ? opts.showCancel || false : false;
        ...
        dialog.innerHTML = [
            ...
            '<div class="sca-dialog-icon">',
            iconHtml(type),
            "</div>",
            "</div>",
            '<div class="sca-dialog-body">',
            '<div class="sca-dialog-title">',
            esc(title),
            "</div>",
            message
                ? '<div class="sca-dialog-msg">' + esc(message) + "</div>"
                : "",
            extraHtml
                ? '<div class="sca-dialog-extra">' + extraHtml + "</div>"
                : "",
            "</div>",
            formHtml
                ? '<div class="sca-dialog-form">' + formHtml + "</div>"
                : "",
            '<div class="sca-dialog-footer' +
                (footerCol ? " sca-footer-col" : "") +
                '">',
            showCancel
                ? '<button class="sca-btn sca-btn-cancel" data-sca="cancel"><span>' +
                  esc(cancelText) +
                  "</span></button>"
                : "",
            '<button class="sca-btn sca-btn-confirm t-' +
                type +
                '" data-sca="confirm">',
            "<span>",
            iconHtml(type),
  ...
  ───────────────────────────────────────────────────── */
    SCA.loading = function (opts) {
        ...
        dialog.innerHTML = [
            ...
            '<div class="sca-dialog-title" style="margin-top:16px">',
            esc(opts.title || "Memuat..."),
            "</div>",
            opts.message
  ...
  ───────────────────────────────────────────────────── */
    SCA.toast = function (opts) {
        ...
        if (
            !_toastContainers[pos] ||
            !document.body.contains(_toastContainers[pos])
        ) {
        ...
        toast.innerHTML = [
            '<div class="sca-toast-ico"><i class="bi ' +
                t.icon +
                '"></i></div>',
            '<div class="sca-toast-content">',
            title
                ? '<div class="sca-toast-title">' + esc(title) + "</div>"
                : "",
            message
                ? '<div class="sca-toast-msg">' + esc(message) + "</div>"
                : "",
            "</div>",
            '<button class="sca-toast-close" aria-label="Close"><i class="bi bi-x-lg"></i></button>',
            duration > 0
                ? '<div class="sca-toast-bar" style="animation-duration:' +
                  duration +
                  'ms"></div>'
                : "",
        ].join("");
  ...
  ───────────────────────────────────────────────────── */
    SCA.alert = function (opts, target) {
        ...
        var container =
            typeof target === "string"
                ...
                : target instanceof Element
                  ...
                  : null;
        if (!container) {
            console.warn("SCA.alert: target not found", target);
            return;
        }
        ...
        banner.innerHTML = [
            ...
            "</div>",
            dismissible
                ? '<button class="sca-al-close" aria-label="Close"><i class="bi bi-x-lg"></i></button>'
                : "",
        ].join("");

        if (dismissible) {
            banner
                .querySelector(".sca-al-close")
                .addEventListener("click", function () {
                    removeEl(banner);
                });
        }
    ...
    /* ─────────────────────────────────────────────────────
     SHORTHAND METHODS
  ───────────────────────────────────────────────────── */
    ["success", "error", "warning", "info", "primary", "dark", "light"].forEach(
        function (type) {
            ...
            Object.assign(
                {
                    ...
                    cancelText: "Batal",
                    extraHtml:
                        '<i class="bi bi-exclamation-triangle-fill" style="color:#f59e0b;margin-right:6px"></i>Semua data terkait akan ikut terhapus.',
                },
                opts || {},
            ),
        );
  ...
  ───────────────────────────────────────────────────── */
    ...
    SCA.activateUser = function (user) {
        ...
        return new Promise(function (resolve) {
            var built = buildDialog({
                ...
                message:
                    "Pengguna akan dapat mengakses sistem setelah diaktifkan.",
                showCancel: true,
            ...
            dialog.addEventListener("click", function (e) {
                ...
                var result = confirmed
                    ? { confirmed: true, note: note ? note.value : "" }
                    : false;
                dialog.classList.add("sca-out");
                setTimeout(function () {
                    closeOverlay(overlay);
            ...
            dialog.addEventListener("click", function (e) {
                ...
                if (confirmed) {
                    ...
                    if (!to || !subj || !body) {
                        ...
                        if (subjEl)
                            subjEl.style.borderColor = !subj ? "#ff4d6d" : "";
                        if (bodyEl)
                            bodyEl.style.borderColor = !body ? "#ff4d6d" : "";
                        return;
                    }
                    result = {
                        confirmed: true,
    ...
    SCA.resetPassword = function (opts) {
        ...
        var formHtml = [
            ...
            '<input type="password" class="sca-input" name="newPassword" id="sca-reset-pw" placeholder="Minimal 8 karakter..." />',
            ...
            '<div class="sca-pw-wrap">',
            '<input type="password" class="sca-input" name="newPasswordConfirmation" id="sca-reset-pw2" placeholder="Ulangi password baru..." />',
            ...
            dialog.addEventListener("click", function (e) {
                ...
                if (confirmed) {
                    ...
                    if (mode === "manual") {
                        ...
                        if (!p1 || p1.length < 8) {
                            if (pw1) pw1.style.borderColor = "#ff4d6d";
                            return;
                        }
                        if (p1 !== p2) {
                            if (pw2) {
                                pw2.style.borderColor = "#ff4d6d";
                                pw2.placeholder = "Password tidak cocok!";
                            }
                            return;
                        }
                        result = {
